...
export default async function UserQuyTrinhQuyDinhLeaderPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const docs = await loadK12LeaderDocs({ includeDraft: false });
  const rawDoc = params.doc;
  const selectedSlug = Array.isArray(rawDoc) ? rawDoc[0] : rawDoc || docs.defaultSlug;

  return (
    <K12DocsClient
      basePath="/user/quy-trinh-quy-dinh-leader"
