...
export default function ManageK12DocsPage() {
  ...
  const callApi = async (
    ...
    const response = await fetch(`/api/k12-leader-docs?${query.toString()}`, {
      method,
      headers: {
  ...
  const topHeaderActions = (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Link href="/admin/quy-trinh-quy-dinh-leader" className="text-sm font-medium text-[#a1001f] hover:underline">
        Xem Trang K12
      </Link>
    </div>
  );
